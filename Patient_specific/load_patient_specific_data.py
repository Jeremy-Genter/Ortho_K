...
an_surf_l = np.zeros([31, 255])
an_surf_r = np.zeros([31, 255])
an_power_l = np.zeros([31, 255])
an_power_r = np.zeros([31, 255])
thickness_l = np.zeros([31, 255])
thickness_r = np.zeros([31, 255])
pos_surf_l = np.zeros([31, 255])
pos_surf_r = np.zeros([31, 255])
...
for loop in range(4):
    ...
    for i in range(31):
        an_surf_l[i, :] = np.asarray(p_data_l['CornealThickness [um]'][i + 32].split(';'))[:-1].astype(float)[:-1]
        an_surf_r[i, :] = np.asarray(p_data_r['CornealThickness [um]'][i + 32].split(';'))[:-1].astype(float)[:-1]
        thickness_l[i, :] = np.asarray(p_data_l['CornealThickness [um]'][i].split(';'))[:-1].astype(float)[:-1]
        thickness_r[i, :] = np.asarray(p_data_r['CornealThickness [um]'][i].split(';'))[:-1].astype(float)[:-1]
        an_power_l[i, :] = np.asarray(p_data_l['CornealThickness [um]'][i + 128].split(';'))[:-1].astype(float)[:-1]
        an_power_r[i, :] = np.asarray(p_data_r['CornealThickness [um]'][i + 128].split(';'))[:-1].astype(float)[:-1]
    ...
    theta = np.linspace(0, 2*np.pi, 256)[:-1]
    r = np.linspace(0, 6, 31)
    theta, r = np.meshgrid(theta, r)
    x, y = pol2cart(r, theta)
    r_out_index = 7
    data_an_l = np.zeros([255*r_out_index, 3])
    ...
    while j < len(data_pos_r[:, 0]):
        if np.sum(np.isnan(data_pos_r[j, :])) > 0:
            ...
            continue
        j += 1
    data_pos_l[:, 2] = data_pos_l[:, 2] - (data_pos_l[:255, 2]).min()
    data_pos_r[:, 2] = data_pos_r[:, 2] - (data_pos_r[:255, 2]).min()
    for n in range(4):
        if n == 0:
            data_temp = data_an_l[255:, :]
        elif n == 1:
            data_temp = data_an_r[255:, :]
        elif n == 2:
            data_temp = data_pos_l[255:, :]
        elif n == 3:
            data_temp = data_pos_r[255:, :]
    ...
    if loop == 0:
        ...
        CS = axs1[1, 0].contourf(x, y, an_power_l, levels=levels, cmap='rainbow')
        cbar = fig1.colorbar(CS, ax=axs1[1, 0])
        cbar.ax.set_ylabel('refractive power [D]')
        axs1[1, 0].set_title('Anterior Surface Power Left Before Treatment')
        axs1[1, 0].set_xlabel('X-Coordinates [mm]')
        axs1[1, 0].set_ylabel('Y-Coordinates [mm]')
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs2[1, 0].contourf(x, y, an_power_r, levels=levels, cmap='rainbow')
        cbar = fig2.colorbar(CS, ax=axs2[1, 0])
        cbar.ax.set_ylabel('refractive power [D]')
        axs2[1, 0].set_title('Anterior Surface Power Left Before Treatment')
        axs2[1, 0].set_xlabel('X-Coordinates [mm]')
        axs2[1, 0].set_ylabel('Y-Coordinates [mm]')
    elif loop == 1:
        levels = np.linspace(0, 3, 150)
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs1[1, 1].contourf(x, y, an_power_l, levels=levels, cmap='rainbow')
        cbar = fig1.colorbar(CS, ax=axs1[1, 1])
        cbar.ax.set_ylabel('refractive power [D]')
        axs1[1, 1].set_title('Anterior Surface Power After Treatment')
        axs1[1, 1].set_xlabel('X-Coordinates [mm]')
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs2[1, 1].contourf(x, y, an_power_r, levels=levels, cmap='rainbow')
        cbar = fig2.colorbar(CS, ax=axs2[1, 1])
        cbar.ax.set_ylabel('refractive power [D]')
        axs2[1, 1].set_title('Anterior Surface Power After Treatment')
        axs2[1, 1].set_xlabel('X-Coordinates [mm]')
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs3[1, 0].contourf(x, y, an_power_l, levels=levels, cmap='rainbow')
        cbar = fig3.colorbar(CS, ax=axs3[1, 0])
        cbar.ax.set_ylabel('refractive power [D]')
        axs3[1, 0].set_title('Anterior Surface Power Left Before Treatment')
        axs3[1, 0].set_xlabel('X-Coordinates [mm]')
        axs3[1, 0].set_ylabel('Y-Coordinates [mm]')
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs4[1, 0].contourf(x, y, an_power_r, levels=levels, cmap='rainbow')
        cbar = fig4.colorbar(CS, ax=axs4[1, 0])
        cbar.ax.set_ylabel('refractive power [D]')
        axs4[1, 0].set_title('Anterior Surface Power Left Before Treatment')
        axs4[1, 0].set_xlabel('X-Coordinates [mm]')
        axs4[1, 0].set_ylabel('Y-Coordinates [mm]')
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs3[1, 1].contourf(x, y, an_power_l, levels=levels, cmap='rainbow')
        cbar = fig3.colorbar(CS, ax=axs3[1, 1])
        cbar.ax.set_ylabel('refractive power [D]')
        axs3[1, 1].set_title('Anterior Surface Power After Treatment')
        axs3[1, 1].set_xlabel('X-Coordinates [mm]')
        ...
        levels = np.linspace(40, 55, 150)
        CS = axs4[1, 1].contourf(x, y, an_power_r, levels=levels, cmap='rainbow')
        cbar = fig4.colorbar(CS, ax=axs4[1, 1])
        cbar.ax.set_ylabel('refractive power [D]')
        axs4[1, 1].set_title('Anterior Surface Power After Treatment')
        axs4[1, 1].set_xlabel('X-Coordinates [mm]')
print(R)
print(0.3375/(R*1e-3))

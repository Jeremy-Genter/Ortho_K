...
for ii in range(len(results)):
    if results['thickness_central 1d'][i] < -4 or results['power_eye 1d'][i] > -0.03*2.25 or results['power_eye 1d'][i] < -0.6*2.25\
            or results['contact time -2 D'][i] > 1*60 or results['contact time -2 D'][i] < 0.15*60 or \
            results['keratometry 8h'][i] > -0.2*2.25 or results['keratometry 8h'][i] < -2.25*0.75:
        rejected = pd.DataFrame.append(rejected, results.iloc[j])
        results = pd.DataFrame.drop(results, i)
    else:
        j +=1
